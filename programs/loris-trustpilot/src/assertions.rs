use crate::{error::LorisTrustpilotError, state::Key};
use solana_program::{
...
pub fn assert_program_owner(
    ...
    if account.owner != owner {
        ...
        Err(LorisTrustpilotError::InvalidProgramOwner.into())
    } else {
        Ok(())
    }
}
...
pub fn assert_pda(
    ...
    if *account.key != key {
        ...
        return Err(LorisTrustpilotError::InvalidPda.into());
    }
    Ok(bump)
}
...
pub fn assert_empty(account_name: &str, account: &AccountInfo) -> ProgramResult {
    if !account.data_is_empty() {
        ...
        Err(LorisTrustpilotError::ExpectedEmptyAccount.into())
    } else {
        Ok(())
    }
}
...
pub fn assert_non_empty(account_name: &str, account: &AccountInfo) -> ProgramResult {
    if account.data_is_empty() {
        ...
        Err(LorisTrustpilotError::ExpectedNonEmptyAccount.into())
    } else {
        Ok(())
    }
}
...
pub fn assert_signer(account_name: &str, account: &AccountInfo) -> ProgramResult {
    if !account.is_signer {
        ...
        Err(LorisTrustpilotError::ExpectedSignerAccount.into())
    } else {
        Ok(())
    }
}
...
pub fn assert_writable(account_name: &str, account: &AccountInfo) -> ProgramResult {
    if !account.is_writable {
        ...
        Err(LorisTrustpilotError::ExpectedWritableAccount.into())
    } else {
        Ok(())
    }
}
...
pub fn assert_same_pubkeys(
    ...
    if account.key != expected {
        ...
        Err(LorisTrustpilotError::AccountMismatch.into())
    } else {
        Ok(())
    }
}
...
pub fn assert_account_key(account_name: &str, account: &AccountInfo, key: Key) -> ProgramResult {
    ...
    if account.data_len() <= 1 || account.try_borrow_data()?[0] != key_number {
        ...
        Err(LorisTrustpilotError::InvalidAccountKey.into())
    } else {
        Ok(())
    }
}
